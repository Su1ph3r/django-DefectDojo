...
# Migration modules are numerically prefixed, so they are not importable by name.
backfill_empty_status = importlib.import_module(
    "dojo.db_migrations.0281_backfill_empty_engagement_status",
).backfill_empty_status
...
class TestBackfillEmptyEngagementStatus(DojoTestCase):

    """
    The 0281 backfill repairs rows already stored, so they recover without waiting for a save.

    The migration's operation is exercised directly rather than through MigratorTestCase:
    this schema cannot be reverse-migrated (the repo's own MigratorTestCase is skipped for
    that reason), and the backfill is a plain queryset update whose behavior does not depend
    on which historical model state it is handed.
    """

    def setUp(self):
        super().setUp()
        product_type, _ = Product_Type.objects.get_or_create(name="backfill")
        self.product, _ = Product.objects.get_or_create(
            name="TestBackfillEmptyEngagementStatus",
            description="Test",
            prod_type=product_type,
        )

    def _engagement(self, name, **columns):
        ...
        # Straight to the columns: the model normalizes these values on save.
        Engagement.objects.filter(pk=engagement.pk).update(**columns)
        return engagement.pk

    def test_empty_values_are_filled_in_and_populated_ones_are_left_alone(self):
        null_status = self._engagement("null status", status=None)
        blank_status = self._engagement("blank status", status="")
        null_type = self._engagement("null type", engagement_type=None)
        populated = self._engagement("populated", status="On Hold", engagement_type="CI/CD")

        backfill_empty_status(apps, None)

        self.assertEqual("Not Started", Engagement.objects.get(pk=null_status).status)
        self.assertEqual("Not Started", Engagement.objects.get(pk=blank_status).status)
        self.assertEqual("Interactive", Engagement.objects.get(pk=null_type).engagement_type)

        untouched = Engagement.objects.get(pk=populated)
        self.assertEqual("On Hold", untouched.status)
        self.assertEqual("CI/CD", untouched.engagement_type)
